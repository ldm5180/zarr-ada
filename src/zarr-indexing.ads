...
package Zarr.Indexing with SPARK_Mode is
   ...
   function Ceil_Div (A, B : Positive) return Positive
     with Pre  => A <= Integer'Last - B,
          Post => Ceil_Div'Result >= 1;
   ...
   function Product (E : Extent_Array) return Long_Long_Integer
     with Pre  => (for all I in E'Range => E (I) >= 1),
          Post => Product'Result >= 1
                  and then Product'Result <= Overflow_Sentinel;
   ...
   function Linear (Coord, Shape : Extent_Array) return Long_Long_Integer
     with Pre  => Coord'First = Shape'First
                  and then Coord'Last = Shape'Last
                  and then (for all I in Coord'Range => Coord (I) < Shape (I)),
          Post => Linear'Result >= 0
                  and then Linear'Result <= Overflow_Sentinel;
